...
juce::AudioProcessorValueTreeState::ParameterLayout MyPlugInAudioProcessor::createParameters() {
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;

    params.push_back(std::make_unique<juce::AudioParameterFloat>("DEPTH", "Depth", 1.0f, 1.059f, 0.001f ));
    params.push_back(std::make_unique<juce::AudioParameterFloat>("RATE", "Rate", 0.1f, 8.0f, 0.1f ));

    params.push_back(std::make_unique<juce::AudioParameterFloat>("DELAY", "Delay", 1.5f, 15.0f, 0.1f ));

    params.push_back(std::make_unique<juce::AudioParameterFloat>("PHASE", "Phase", 0.0f, 180.0f, 1.0f ));

    params.push_back(std::make_unique<juce::AudioParameterFloat>("DRYWET", "Dry/Wet", 0.0f, 1.0f, 0.4f ));

    params.push_back(std::make_unique<juce::AudioParameterFloat>("REGEN", "Regen", 0.0f, 0.95f, 0.01f ));
